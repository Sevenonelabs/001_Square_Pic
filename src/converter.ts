import './style.css';
import { encodeGIF, encodeICO, encodeTIFF } from './encoders';
...
interface FileItem {
  ...
  targetFormat: 'jpeg' | 'png' | 'webp' | 'bmp' | 'gif' | 'ico' | 'avif' | 'tiff';
...
// Detect when source and target formats match (skip re-encoding)
function sourceMatchesTargetFormat(file: File, targetFormat: string): boolean {
  ...
  if (mimeToFormat[file.type] === targetFormat) return true;

  const dot = file.name.lastIndexOf('.');
  if (dot !== -1) {
    const ext = file.name.substring(dot + 1).toLowerCase();
    return (ext === 'jpg' ? 'jpeg' : ext) === targetFormat;
  }
  return false;
}
...
async function convertAllFiles() {
  ...
  for (let i = 0; i < filesList.length; i++) {
    ...
    try {
      ...
      // Same-format: pass through original file without re-encoding
      if (sourceMatchesTargetFormat(item.file, item.targetFormat)) {
...
function convertLocalCanvas(
  ...
  return new Promise((resolve, reject) => {
    ...
    // Custom encoders for formats not supported by canvas.toBlob
    if (format === 'gif') {
      const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
      resolve(encodeGIF(imageData));
      return;
    }
    ...
    if (format === 'ico') {
      canvas.toBlob((pngBlob) => {
        if (pngBlob) {
          resolve(encodeICO(pngBlob));
        } else {
          reject(new Error('ICO encoding failed: could not create intermediate PNG'));
        }
      }, 'image/png');
      return;
    }
    ...
    if (format === 'tiff') {
      const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
      resolve(encodeTIFF(imageData));
      return;
    }

    // Standard formats supported natively by canvas.toBlob
    let mime = 'image/jpeg';
    ...
    canvas.toBlob((blob) => {
      if (blob) {
        ...
        reject(new Error(`Your browser does not support ${mime.split('/')[1].toUpperCase()} encoding.`));
      }
    }, mime, qualityArg);
  });
}
...
function positionPopover(trigger: HTMLElement) {
  ...
  // Make sure popover stays inside screen boundary (viewport-relative check)
  const rightEdge = left - window.scrollX + popoverWidth;
  if (rightEdge > viewportWidth) {
    left = viewportWidth + window.scrollX - popoverWidth - 16;
  }
  if (left < 16) {
    left = 16;
  }
    ...
    <div class="popover-body">
      ...
      <div class="popover-right">
        <div class="formats-grid" id="formats-grid-el">
          ...
          <button class="format-btn ${currentSelection === 'avif' ? 'active' : ''}" data-fmt="avif">AVIF</button>
          <button class="format-btn ${currentSelection === 'tiff' ? 'active' : ''}" data-fmt="tiff">TIFF</button>
        </div>
      </div>
    </div>
  `;
