import"./modulepreload-polyfill-CH8_0oRz.js";import"./main-DuwQSqGA.js";var e=[],t=`webp`,n=null,r=null,i=null,a=null,o=null,s=null,c=null,l=null,u=null,d=null,f=null,p=null,m=null,h=null;function g(){_(),v(),C()}document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,g):g();function _(){r=document.getElementById(`converter-drop-zone`),i=document.getElementById(`select-files-btn`),a=document.getElementById(`converter-file-input`),o=document.getElementById(`converter-workspace`),s=document.getElementById(`files-count-badge`),c=document.getElementById(`add-more-btn`),l=document.getElementById(`bulk-format-btn`),u=document.getElementById(`bulk-format-label`),d=document.getElementById(`converter-files-list`),f=document.getElementById(`clear-all-btn`),p=document.getElementById(`convert-all-btn`),m=document.getElementById(`download-all-btn`),document.getElementById(`format-popover`)?h=document.getElementById(`format-popover`):(h=document.createElement(`div`),h.id=`format-popover`,h.className=`format-popover`,h.style.display=`none`,document.body.appendChild(h))}function v(){i&&a&&i.addEventListener(`click`,e=>{e.stopPropagation(),a.click()}),a&&(a.addEventListener(`change`,y),a.addEventListener(`click`,e=>{e.stopPropagation()})),r&&(r.addEventListener(`click`,()=>{a?.click()}),r.addEventListener(`dragover`,e=>{e.preventDefault(),r.classList.add(`drag-over`),r.style.borderColor=`var(--accent)`,r.style.background=`var(--surface-mid)`}),r.addEventListener(`dragleave`,()=>{r.classList.remove(`drag-over`),r.style.borderColor=`var(--border-color)`,r.style.background=`rgba(0,0,0,0.15)`}),r.addEventListener(`drop`,e=>{e.preventDefault(),r.classList.remove(`drag-over`),r.style.borderColor=`var(--border-color)`,r.style.background=`rgba(0,0,0,0.15)`;let t=e.dataTransfer?.files;t&&t.length>0&&b(t)})),c&&c.addEventListener(`click`,()=>{a?.click()}),l&&l.addEventListener(`click`,e=>{e.stopPropagation(),N(`bulk`,l)}),f&&f.addEventListener(`click`,D),p&&p.addEventListener(`click`,k),m&&m.addEventListener(`click`,M),document.addEventListener(`click`,e=>{if(h&&h.style.display!==`none`){let t=e.target;!h.contains(t)&&(!n||!n.element.contains(t))&&F()}}),window.addEventListener(`resize`,()=>{h&&h.style.display!==`none`&&n&&P(n.element)})}function y(e){let t=e.target.files;t&&t.length>0&&b(t)}function b(n){let r=!1;for(let i=0;i<n.length;i++){let a=n[i];if(!a.type.startsWith(`image/`)){alert(`"${a.name}" is not a valid image file.`);continue}if(a.size>30*1024*1024){alert(`"${a.name}" is too large. Max size is 30MB.`);continue}let o=Math.random().toString(36).substring(2,11),s=URL.createObjectURL(a),c={id:o,file:a,name:a.name,size:a.size,src:s,imgElement:null,targetFormat:t,quality:.9,status:`ready`,convertedBlob:null,settingsOpen:!1},l=new Image;l.onload=()=>{c.imgElement=l},l.src=s,e.push(c),r=!0}r&&C(),a&&(a.value=``)}function x(e,t=2){if(e===0)return`0 Bytes`;let n=1024,r=t<0?0:t,i=[`Bytes`,`KB`,`MB`,`GB`],a=Math.floor(Math.log(e)/Math.log(n));return parseFloat((e/n**+a).toFixed(r))+` `+i[a]}function S(e,t=28){if(e.length<=t)return e;let n=Math.floor(t/2)-1;return e.substring(0,n)+`...`+e.substring(e.length-n)}function C(){let t=document.getElementById(`converter-tool`),n=document.getElementById(`drop-zone-title`);t&&t.classList.add(`has-files`),o&&(o.style.display=`block`),r&&(r.style.display=`flex`),n&&(n.textContent=`Drag & Drop More`),s&&(s.textContent=`${e.length} file${e.length>1?`s`:``}`),w(),O()}function w(){let t=d;if(t){if(t.innerHTML=``,e.length===0){t.innerHTML=`
    ...
    `;return}e.forEach(e=>{let n=document.createElement(`div`);n.className=`file-row`,n.dataset.id=e.id;let r=`ready`,i=`Ready`;e.status===`converting`?(r=`converting`,i=`Converting`):e.status===`done`?(r=`done`,i=`Ready to Download`):e.status===`error`&&(r=`error`,i=`Error`),n.innerHTML=`
      <div class="file-row-main" style="display: flex; align-items: center; width: 100%; gap: 0.8rem; flex-wrap: nowrap;">
        ...
        <div class="file-row-info" style="flex: 1; min-width: 0;">
          <div class="file-row-name" title="${e.name}">${S(e.name)}</div>
    ...
    `;let a=n.querySelector(`.file-row-format-btn`);a.addEventListener(`click`,t=>{t.stopPropagation(),N(e.id,a)}),n.querySelector(`.gear-btn`).addEventListener(`click`,t=>{t.stopPropagation(),T(e.id)});let o=n.querySelector(`.row-quality-slider`),s=n.querySelector(`.row-quality-val`);o.addEventListener(`input`,()=>{e.quality=parseInt(o.value)/100,s.textContent=`${o.value}%`,e.status===`done`&&(e.status=`ready`,w())}),e.status===`done`&&n.querySelector(`.download-btn`).addEventListener(`click`,t=>{t.stopPropagation(),j(e)}),n.querySelector(`.remove-btn`).addEventListener(`click`,t=>{t.stopPropagation(),E(e.id)}),t.appendChild(n)})}}function T(t){let n=e.find(e=>e.id===t);n&&(n.settingsOpen=!n.settingsOpen,w())}function E(t){let n=e.findIndex(e=>e.id===t);n!==-1&&(URL.revokeObjectURL(e[n].src),e.splice(n,1),C())}function D(){e.forEach(e=>URL.revokeObjectURL(e.src)),e=[],F(),C()}function O(){if(!(!p||!m||!f)){if(e.length===0){p.style.display=`flex`,m.style.display=`none`,p.querySelector(`span`).textContent=`Convert Files`,p.disabled=!0,p.style.opacity=`0.5`,p.style.cursor=`not-allowed`,f.disabled=!0,f.style.opacity=`0.5`,f.style.cursor=`not-allowed`;return}if(p.disabled=!1,p.style.opacity=`1`,p.style.cursor=`pointer`,f.disabled=!1,f.style.opacity=`1`,f.style.cursor=`pointer`,e.every(e=>e.status===`done`))p.style.display=`none`,m.style.display=`flex`,m.textContent=`Download All (${e.length} files)`;else{p.style.display=`flex`,m.style.display=`none`;let t=e.filter(e=>e.status!==`done`).length;p.querySelector(`span`).textContent=`Convert ${t} File${t>1?`s`:``}`}}}async function k(){if(e.length!==0){p.disabled=!0,p.querySelector(`span`).textContent=`Converting...`,p.style.opacity=`0.7`;for(let t=0;t<e.length;t++){let n=e[t];if(n.status!==`done`){n.status=`converting`,w();try{n.imgElement||await new Promise(e=>{let t=new Image;t.onload=()=>{n.imgElement=t,e()},t.src=n.src}),n.convertedBlob=await A(n.imgElement,n.targetFormat,n.quality),n.status=`done`}catch(e){console.error(e),n.status=`error`}w()}}p.disabled=!1,p.style.opacity=`1`,O()}}function A(e,t,n){return new Promise((r,i)=>{let a=document.createElement(`canvas`);a.width=e.naturalWidth,a.height=e.naturalHeight;let o=a.getContext(`2d`);if(!o){i(Error(`2D context fail`));return}t===`jpeg`&&(o.fillStyle=`#FFFFFF`,o.fillRect(0,0,a.width,a.height)),o.drawImage(e,0,0);let s=`image/jpeg`;t===`png`?s=`image/png`:t===`webp`?s=`image/webp`:t===`bmp`?s=`image/bmp`:t===`gif`?s=`image/gif`:t===`ico`&&(s=`image/x-icon`),a.toBlob(e=>{e?r(e):i(Error(`Canvas Blob creation failed`))},s,t===`png`?void 0:n)})}function j(e){if(!e.convertedBlob)return;let t=e.targetFormat===`jpeg`?`jpg`:e.targetFormat,n=`${e.name.substring(0,e.name.lastIndexOf(`.`))||e.name}.${t}`,r=document.createElement(`a`);r.download=n,r.href=URL.createObjectURL(e.convertedBlob),document.body.appendChild(r),r.click(),document.body.removeChild(r),setTimeout(()=>URL.revokeObjectURL(r.href),1e3)}function M(){e.forEach((e,t)=>{e.status===`done`&&e.convertedBlob&&setTimeout(()=>{j(e)},t*250)})}function N(e,t){if(h&&h.style.display!==`none`&&n&&n.id===e){F();return}n={id:e,element:t},I(e),h&&(h.style.display=`flex`,P(t))}function P(e){if(!h)return;let t=e.getBoundingClientRect(),n=t.left+window.scrollX,r=t.bottom+window.scrollY+6;n+320>window.innerWidth&&(n=window.innerWidth-320-16),n<16&&(n=16),r+248>window.innerHeight+window.scrollY&&(r=t.top+window.scrollY-248-6),h.style.left=`${n}px`,h.style.top=`${r}px`}function F(){h&&(h.style.display=`none`),n=null}function I(n){if(!h)return;let r=`jpeg`;if(n===`bulk`)r=t;else{let t=e.find(e=>e.id===n);t&&(r=t.targetFormat)}h.innerHTML=`
    <!-- Search Bar -->
  ...
  `;let i=h.querySelector(`#format-search`),a=h.querySelector(`#formats-grid-el`);i&&a&&(i.focus(),i.addEventListener(`input`,()=>{let e=i.value.toLowerCase().trim();a.querySelectorAll(`.format-btn`).forEach(t=>{let n=t.textContent.toLowerCase(),r=t;n.includes(e)?r.style.display=`block`:r.style.display=`none`})})),h.querySelectorAll(`.format-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation(),L(n,e.getAttribute(`data-fmt`)),F()})})}function L(n,r){if(n===`bulk`)t=r,u&&(u.textContent=r.toUpperCase()),e.forEach(e=>{e.targetFormat!==r&&(e.targetFormat=r,e.status===`done`&&(e.status=`ready`,e.convertedBlob=null))});else{let t=e.find(e=>e.id===n);t&&t.targetFormat!==r&&(t.targetFormat=r,t.status===`done`&&(t.status=`ready`,t.convertedBlob=null))}C()}